...
We will store each project MDX entry at `src/content/projects/<locale>/<slug>/index.mdx` and store images shared by both locales at `src/assets/projects/<slug>/images/`.

## Consequences

Each locale owns a complete project entry under the same directory structure, while shared images have one source file. Relative image paths cross from the content directory into the shared asset directory. The `projectsContent` glob must match `*/*/index.mdx`; Astro removes the trailing `/index` from the generated ID, so existing IDs remain `<locale>/<slug>` and current route and locale filters remain valid.
