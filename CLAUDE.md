...
- `src/components/analytics.astro` — config-gated cookieless Umami `<script is:inline>`, emitted only when `UMAMI_WEBSITE_ID` is set (`astro:env/client`; `UMAMI_HOST` defaults to `https://cloud.umami.is`). Inert (no script, no request) otherwise. The id lives in env (gitignored `.env` locally + Cloudflare Pages env in prod), never in source.
- `astro.config.mjs` — sets `site` (`https://adeonir.dev`), the `@astrojs/sitemap` integration (`filter` drops `/styleguide`), and the `env.schema` for the two public client Umami vars, alongside the dev-only `landing()` integration.
