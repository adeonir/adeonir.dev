...
- **External services:** Resend (outbound email), a hosted mailbox on `adeonir.dev` (inbound `contato@adeonir.dev`).
...
- `src/components/analytics.astro` — analytics mount point, currently inert (renders nothing — no script, no request).
- `astro.config.mjs` — sets `site` (`https://adeonir.dev`), the `@astrojs/sitemap` integration (`filter` drops `/styleguide`), and an empty `env.schema`, alongside the dev-only `landing()` integration.
