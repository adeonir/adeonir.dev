...
- `pnpm deploy:preview` — same build, deployed to the `preview` branch (preview URL on `*.adeonir.pages.dev`).
...
- `astro.config.mjs` — sets `site` (`https://adeonir.dev`), the `@astrojs/sitemap` integration (`filter` drops `/styleguide`), and the `env.schema` for the two public client PostHog vars (`POSTHOG_KEY` optional — gates emission; `POSTHOG_HOST` defaults to `https://t.adeonir.dev`, the first-party reverse proxy), alongside the dev-only `landing()` integration.
