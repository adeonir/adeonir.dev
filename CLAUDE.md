...
Deploys are **manual** via Wrangler against the Cloudflare Pages **direct-upload** project `adeonir` (not Git-connected — no push-triggered CI). Project `name` and output dir (`./dist`) live in `wrangler.jsonc`, so the scripts only pass `--branch`. The scripts are named `deploy:*` (not bare `deploy`) because `pnpm deploy` collides with pnpm's built-in workspace-deploy command. `pnpm-workspace.yaml` must keep `allowBuilds.workerd: true` — an unresolved value there hard-crashes **every** `pnpm run` via the pre-run deps check.
...
- `wrangler.jsonc` — Cloudflare Pages config for manual deploys: `name` (`adeonir`), `pages_build_output_dir` (`./dist`), `compatibility_date`. Lets `pnpm deploy:prod`/`deploy:preview` run bare `wrangler pages deploy --branch=…`. Direct-upload project; no Git connection.
...
- **Landing surface (dev + preview)** — three files cooperate so the landing renders at `/` in dev and in preview builds without shipping to production: `src/pages/_landing.astro` (the stub; `_`-prefix keeps it out of the prod build), the `landing()` integration in `astro.config.mjs` (injects the `/_landing` route under `astro dev` or when `PREVIEW=true`), and `src/middleware.ts` (rewrites `/` → `/_landing` when `import.meta.env.DEV` or the `PREVIEW` flag is set). All no-op in a normal build, where `/` stays the holding page. The build content (`PREVIEW`) and the deploy target (`--branch`) are independent axes — `pnpm deploy:landing` builds with `PREVIEW=true` and publishes to the `preview` branch, so the landing reaches the preview deployment while `deploy:preview` still ships the holding page there. Preview deployments are kept out of search indexes by Cloudflare's default `X-Robots-Tag: noindex`. Throwaway — retire when the landing becomes the prod home.
