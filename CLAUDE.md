...
- **Content layer** under `src/content/`: the scaffolding is wired (`src/content.config.ts` + `src/content/schemas.ts`) with a `settings` collection (site metadata via the `file()` loader). Per-section YAML copy collections (`hero`/`about`/`stack`/`header`/`footer`) are now built on the same scaffolding, validated by the shared `schemas.ts` (Zod). Still planned: a `projects` MDX collection (one case study per folder, colocated images).
